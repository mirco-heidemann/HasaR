...
##for (i in 1) {
  ...
  eps.file <- paste(pdf.out, datum[i], '.meteo.eps', sep = '')
  cairo_ps(file=eps.file, width=8.5, height=5)
...
#                      n, ')', sep = ''), line = 0.4, cex.main = 0.7)
  ...
  eps.file <- paste(pdf.out, datum[i], '.schaden.eps', sep = '')
  cairo_ps(file=eps.file, width=8.5, height=5)
  par(mar = c(1, 1, 2, 1), mfcol = c(1, 2))
...
#   title(main = paste('Ereignis vom ', dat.character, ' - Anzahl Schäden',sep = ''),
