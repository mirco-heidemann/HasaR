...
##  Unterscheidung nach kleinen oder grossen Events.
##
##  1) Gegitterte GemDat Daten laden
##     (aus r-Skript "Aggregate.GemDat.MeteoSwiss.Grid.R", jährlich gerechnet)
...
##
##  DIE RADARBILDER WIE FOLGT ABLEGEN: 
##  Ordner:    ./data/radarTiffs
##  Bennenung: poh.yyyymmdd.tif" und "meshs.yyyymmdd.tif
##
##  HasaR 3.0, Mirco Heidemann, Feb 18, modified July 2021
##
##
## NOTE!!! This skript is encoded with UTF-8. Please change the default text encoding
## accordingly if the text is not displayed correctly:
...
# ## Das dauert länger...
# sh_avzh <-readOGR(paste0(pth_shp, 'AVZH_Gebaeude.shp'),
#                   layer = 'AVZH_Gebaeude')
...
# ## 1. Quelle: analyse.hagelevents.xlsx, Nov 2017, MHE
# kat_1 <- 3e-2
# kat_2 <- 6e-2
# kat_3 <- 8e-2
...
tbl <- data.frame(anz_kat_1 = round(sum(df_interpol$baujahr_kat == 1) *kat_1, 0),
  ...
  mutate(schad_kat_1 = anz_kat_1 *schad_mean,
         ...
         anzahl_hagel = anz_kat_1 + anz_kat_2 + anz_kat_3,
...
## ---------------------------
## Schadenschaetzung nach HasaR:
message(paste0('\n',
                     '\n------------',
                     '\n',
                     'HASAR - Schadenschätzung für das Hagelereignis vom ',
                     eventDat, ': CHF ',
                     round(df_estimates$best_estimate_loss/1e6, 1), ' Mio. bei rund ',
                     format(round(df_estimates$best_estimate_caims), big.mark = "'"),
                     ' vergüteten Schäden.'))

message(paste0('\n',
                     'HASAR - Mit einer Wahrscheinlichkeit von 95% liegt der Schaden zwischen: CHF ',
                     round(df_estimates$lower_loss/1e6, 1), ' Mio. und ',
                     round(df_estimates$upper_loss/1e6, 1), ' Mio.'))

## Schadenschaetzung nach INTERPOL:
message(paste0('\n',
                     '\n',
                     'INTERPOL - Schadenschätzung für das Hagelereignis vom ',
                     eventDat, ': CHF ',
...
df_out <- tribble(
  ...
  "HASAR_ESTIM", df_estimates$best_estimate_loss,  round(df_estimates$best_estimate_caims),
