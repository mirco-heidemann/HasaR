...
load(here(pth_rdata, 'mLogit.rda'))
load(here(pth_rdata, 'mGamma.rda'))
load(here(pth_rdata, 'mPois.rda'))
...
poh <- readGDAL(here(pth_tif, poh_name))
meshs <- readGDAL(here(pth_tif,meshs_name))
...
## der GVZ die Information zu möglichen Hagelschaeden.
...
df_estimates <- data.frame(lower_loss = sum(newdat$estimated_loss_ll),
                       ...
                       lower_claims = round(sum(newdat$estimated_claims_ll)),
                       best_estimate_claims = round(sum(newdat$estimated_claims)),
                       upper_claims = round(sum(newdat$estimated_claims_ul)))

## Schadenschaetzung ausgeben:
cat(paste0('HASAR - Der geschätzte Schaden beträgt: ',
                     round(df_estimates$best_estimate_loss/1e6, 1), ' Mio. CHF bei rund ',
                     format(round(df_estimates$best_estimate_claims), big.mark = "'"),' Anzahl Schäden.'))
cat(paste0('HASAR - Der geschätzte Schaden liegt zwischen: ',
...
source(here(pth_func, 'f.gplotData.R'))

## shapes fuer plots einlesen
sh_gemeinde <- readOGR(here(pth_shp, 'gemeinden.2016.shp'), layer = 'gemeinden.2016')
sh_fluss <-readOGR(here(pth_shp, 'Fluesse_gross.shp'), layer = 'Fluesse_gross')
sh_seen <-readOGR(here(pth_shp, 'seendet_250.shp'), layer = 'seendet_250')
sh_Flug <-readOGR(here(pth_shp, 'AVZH_Flughafen_Kloten.shp'),
                  layer = 'AVZH_Flughafen_Kloten')
sh_bezirke <- readOGR(here(pth_shp, 'Bezirke.shp'), layer = 'Bezirke')
sh_schaetzkreis <- readOGR(here(pth_shp, 'gvz_schaetzkreise.shp'),
                           layer = 'gvz_schaetzkreise')
shp_kanton    <- readOGR(here(pth_shp, 'Knt_Umriss wgs84.shp'),
...
ggsave(ggPoh, filename = here(pth_pres, "poh.pdf"), device = cairo_pdf,
       width = width_plot, height = height_plot, units = "in")

cat(paste0('Das POH-Radarbild wurde in folgendem Ordner gespeichert:',
                     here(pth_pres, "poh.pdf")))
...
ggsave(ggSchad, filename = here(pth_pres, "Auswirkung.pdf"), device = cairo_pdf,
       width = width_plot, height = height_plot, units = "in")

cat(paste0('Die Schadengrid Grafik wurde in folgendem Ordner gespeichert: ',
                     here(pth_pres, "Auswirkung.pdf")))
...
ggsave(ggSchadGem, filename = here(pth_pres, "Betroffene_Gemeinden.pdf"),
...
cat(paste0('Die Grafik "Schaden pro Gemeinde" wurde in folgendem Ordner gespeichert:',
                     here(pth_pres, "Betroffene_Gemeinden.pdf")))
...
#### Schadenschaetzung nach der Impact Methode (Haimo)
...
Die Impact Funktion leitet sich aus Analysen vergangener Hagelereignisse ab, welche bei der Entwicklung von Haimo durchgeführt wurden.

*Lineare Hail Impact Function GVZ:*
...
*Constant Hail Impact Function GVZ:*
0.000146 for all MESHS Value

```{r warning = FALSE}
## Lineare Impact Function von Haimo
tbl_impfct_lin <- data.frame(meshs_class = seq(from=0, to=10, by=1),
                             meshs = seq(from=1.5, to=6.5, by=0.5),
                             impact = c(0,
                                        0.0000823,
                                        0.0000894,
...
## Konstante Impact Function von Haimo
tbl_impfct_lin <- data.frame(meshs_class = seq(from=0, to=10, by=1),
                         meshs = seq(from=1.5, to=6.5, by=0.5),
                         impact = rep(0.000146, times = 11))


## definieren das Objekt mit den Radar-Daten fuer interp.surface
loc_obj <- cbind(df_exp_lv03$geox, df_exp_lv03$geoy)
...
df_interpol <- df_exp_lv03 %>% dplyr::select(versSum, geox, geoy) %>% 
  ## interpolation der poh und meshs werte auf die gebaeude
  mutate(poh = round(interp.surface(grid_obj_poh, loc_obj),1) *10,
         meshs_class = round(interp.surface(grid_obj_meshs, loc_obj))) %>%
  ## nur Gebäude mit mindestens poh 80%
  filter(poh >= 80) %>% 
  left_join(tbl_impfct_lin, by = 'meshs_class') %>% 
  mutate(schaden_impact = versSum * impact)

sum(df_interpol$schaden_impact)
```
...
df_out <- tribble(
  ...
  "Impact", sum(df_interpol$schaden_impact), NA)
...
cat(paste0('- Hagelereignis vom ', eventDat, ': CHF ',
...
cat("\n")
cat("\n---\n")
cat("Schadenschätzung nach Impact Methode:\n")
cat(paste0('- Hagelereignis vom ', eventDat, ': CHF ',
           format(sum(df_interpol$schaden_impact)/1e6, big.mark="'"), ' Mio.'))
cat("\n---")
```
